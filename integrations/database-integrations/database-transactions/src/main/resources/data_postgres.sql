...
--               employee, project, department CASCADE;
...
-- ═════════════════════════════════════════════════════════════
-- 7. RESET HIBERNATE SEQUENCES
--    Hibernate 6 + Spring Boot 3 uses per-entity sequences with
--    allocationSize=50 (default).  Setting the sequence well
--    above the highest manually-inserted ID prevents PK clashes
--    when the application creates new rows.
--
--    If Hibernate created different sequence names, adjust below
--    (check \ds in psql to list all sequences).
-- ═════════════════════════════════════════════════════════════
DO $$
BEGIN
  -- Department sequence
  IF EXISTS (
      ...
      WHERE schemaname = 'public' AND sequencename = 'department_seq'
  ) THEN
    PERFORM setval('department_seq', 1000);
  END IF;

  -- Employee sequence
  IF EXISTS (
      SELECT 1 FROM pg_sequences
      WHERE schemaname = 'public' AND sequencename = 'employee_seq'
  ) THEN
    PERFORM setval('employee_seq', 1000);
  END IF;
END $$;
